...
def UpdateAction(args, root_key, step_name, action_data):
    command = [
        sys.executable,
        args.cli_path,
        "update",
        ...
        args.state_file,
        "--path",
        "$.{}.steps.{}".format(root_key, step_name),
        "--json-data",
    ...
    try:
        ...
        WriteLog("CLI update timed out after {} seconds for root key {} action {}: {}".format(CLI_COMMAND_TIMEOUT_SECONDS, root_key, step_name, error), "ERROR")
        return False
    if result.returncode != 0:
        WriteLog("CLI update failed for root key {} action {} with code {}".format(root_key, step_name, result.returncode), "ERROR")
...
def HandleLoadInsightData(root_key, alert_data, action_data):
    ...
    try:
        insight_id_data = alios.FindStepByName(alert_data["steps"], "resolveInsightId")
...
def ProcessPackage(args, root_key, alert_data, counters, handlers):
    ...
    for step_name in step_order:
        step_data = action_dictionary[step_name]
        if not isinstance(step_data, dict):
            counters["structure_errors"] += 1
            WriteLog("Action step {} is not a dictionary for root key {}".format(step_name, root_key), "ERROR")
            return
        step_name = step_data.get("stepName")
        module_name = step_data.get("moduleName")
        WriteLog("Current step for root key {}: {}.{}".format(root_key, module_name, step_name), "INFO")
        ...
        if handler is None:
            ...
            return
        step_data["stepNumber"] = step_name
        try:
        ...
        if not UpdateAction(args, root_key, step_name, new_step_data):
            counters["cli_write_errors"] += 1
            return
        action_dictionary[step_name] = new_step_data
        if success:
